...
@router.post("/ob_shared_cdr_report_old")
...
@router.post("/ivr_report_old")
def get_ivr_report_old(
    request: OBCDRReportRequest,
...
@router.post("/ivr_funnel_report_old")
...
@router.get("/after-hours-calls_old")
...
@router.post("/rl_internal_report_old")
def rl_internal_report_old(
    from_date: date,
    to_date: date,
...
@router.post("/company_consumption_month_old")
def company_consumption_month_old(
    request: dict,
    db: Session = Depends(get_db4)
):
...
@router.post("/outbound/Report_old")
def outbound_Report_old(
    company_id: int,
    start_date: date,
    end_date: date,
...
def generate_outbound_excel_old(company_id, start_date, db, db2):

    # call your existing function logic
    data = outbound_Report_old(company_id, start_date, start_date, db, db2)
    ...
    ob_cdr_rows = get_ob_cdr_report_old(request, db, db2)
...
@router.get("/closer-log-report_old")
def closer_log_report_old(
...
@router.get("/closer-log-report-by-campaign_old")
def closer_log_report_by_campaign_old(
    start_date: date = Query(...),
    end_date: date = Query(...),
...
@router.get("/closer-log-report/excel_old")
def closer_log_report_excel_old(
    start_date: date = Query(...),
    end_date: date = Query(...),
    is_shared: str = Query("all"),
    db: Session = Depends(get_db2),
    ...
    # -------------------------------
    response = closer_log_report_old(start_date, end_date, is_shared, db, db4)
    ...
    # -------------------------------
    campaign_response = closer_log_report_by_campaign_old(start_date, end_date, is_shared, db, db4)
...
@router.get("/abandon-callback-report_old")
...
@router.get("/abandon-callback-report/excel_old")
def abandon_callback_report_excel_old(
    ...
    response = abandon_callback_report_old(client_id, report_date, db, db4)
    data = response["data"]
