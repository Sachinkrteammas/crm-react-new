...
const ManageFields = () => {
  ...
  // Auto-select client for Client user or single company
  useEffect(() => {
    if (userType === "Client") {
  ...
  // Fetch fields when client changes
  useEffect(() => {
  ...
  const fetchFields = async () => {
    ...
    try {
      const res = await api.get(`/field_master/fields/${selectedClient}`);
      setFields(res.data);
    } catch (err) {
  ...
  const fetchValues = async (fieldId) => {
    try {
      const res = await api.get(`/field_master/field_values/${selectedClient}/${fieldId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let fieldId = editingField ? editingField.id : null;

      if (editingField) {
        await api.put(`/field_master/fields/${selectedClient}/${fieldId}`, payload);
        if (form.FieldType === "DropDown") {
          // Fetch existing values first
          const existing = await api.get(`/field_master/field_values/${selectedClient}/${fieldId}`);
          const existingMap = new Map(existing.data.map(v => [v.id, v.FieldValueName]));

          // 1. Update or add new
          for (const val of form.DropDownValues.filter(v => v.trim() !== "")) {
            const found = [...existingMap.entries()].find(([, name]) => name === val);
            if (!found) {
              // New → insert
              await api.post(`/field_master/field_values/${selectedClient}`, {
                FieldValueName: val,
                FieldId: fieldId,
              });
            }
          ...
          // 2. Delete removed
          for (const [id, name] of existingMap.entries()) {
            if (!form.DropDownValues.includes(name)) {
              await api.delete(`/field_master/field_values/${selectedClient}/${fieldId}/${id}`);
            }
          }
        }
        alert("Field updated successfully!");
      } else {
        const res = await api.post(`/field_master/fields/${selectedClient}`, payload);
        fieldId = res.data.field_id;
        if (form.FieldType === "DropDown" && form.DropDownValues.length > 0) {
          for (const val of form.DropDownValues.filter(v => v.trim() !== "")) {
            await api.post(`/field_master/field_values/${selectedClient}`, { FieldValueName: val, FieldId: fieldId });
          }
        }
      ...
      console.error(err);
      alert("Error: " + (err.response?.data?.detail || "Failed to save field"));
    }
  };
  ...
  const handleEdit = async (field) => {
      ...
      if (field.FieldType === "DropDown") {
        try {
          const res = await api.get(`/field_master/field_values/${selectedClient}/${field.id}`);
      ...
      setForm({
        ...
        DropDownValues: dropDownValues.length > 0 ? dropDownValues : [],
      });
  ...
  const handleDelete = async (fieldId) => {
    ...
    try {
      await api.delete(`/field_master/fields/${selectedClient}/${fieldId}`);
  ...
  const handleValueSubmit = async (fieldId) => {
    const payload = { FieldValueName: valueForm.FieldValueName, FieldId: fieldId };
    try {
      if (editingValue) {
        await api.put(`/field_master/field_values/${selectedClient}/${editingValue.id}`, payload);
        alert("Value updated!");
      } else {
        await api.post(`/field_master/field_values/${selectedClient}`, payload);
        alert("Value added!");
      }
      setValueForm({});
  ...
  const handleValueDelete = async (fieldId, valueId) => {
    if (!window.confirm("Delete this value?")) return;
    try {
      await api.delete(`/field_master/field_values/${selectedClient}/${fieldId}/${valueId}`);
      fetchValues(fieldId);
    } catch {
        ...
        {selectedClient ? (
          <>
                  ...
                  {fields.length > 0 ? (
                    fields.map((field) => (
                      <tr key={field.id}>
                        ...
                        <td className="text-center">
                          <button className="btn btn-sm btn-outline-warning me-2" onClick={() => handleEdit(field)}>✏ Edit</button>
                          <button className="btn btn-sm btn-outline-danger me-2" onClick={() => handleDelete(field.id)}>🗑 Delete</button>
                          <button
                    ...
                    <tr><td colSpan="7" className="text-center text-muted py-3">No fields found</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            {activeFieldForValues && (
              <div className="mt-4">
                <h5>Field Values for {fields.find(f => f.id === activeFieldForValues)?.FieldName}</h5>
                  ...
                  {(values[activeFieldForValues] || []).map(val => (
                    <li key={val.id} className="list-group-item d-flex justify-content-between align-items-center">
                      ...
                      <span>
                        ...
                        <button className="btn btn-sm btn-outline-danger" onClick={() => handleValueDelete(activeFieldForValues, val.id)}>🗑</button>
                      </span>
                    </li>
                  ))}
                </ul>
                ...
                <div className="modal-dialog modal-lg">
                  <div className="modal-content">
                    <div className="modal-header">
                      <h5 className="modal-title">{editingField ? "Edit Field" : "Add Field"}</h5>
                    ...
                    <div className="modal-body">
                      <form className="row g-3" onSubmit={handleSubmit}>
                        <div className="col-md-6">
                          ...
                          <input name="FieldName" className="form-control" value={form.FieldName} onChange={handleChange} />
                        </div>
                        <div className="col-md-6">
          ...
          (userType === "Super-Admin" || userType === "Admin") && (
            <p className="text-muted">Please select a client to manage fields.</p>
          )
        )}
      </div>
    </div>
