                ...
                <tbody>
                  {reportData.map((site, idx) => (
                    <React.Fragment key={idx}>
                      {site.corrections.map((corr, i) => (
                        <tr key={i}>
                          {i === 0 && <td rowSpan={site.corrections.length}>{site.site !="null"? site.site: ""}</td>}
                          <td>{corr.category !="null"? corr.category: ""}</td>
                          <td>{corr.total}</td>
                          <td>{corr.open !=0 ? corr.open: ""}</td>
                          <td>{corr.close}</td>
                          <td></td>
                        </tr>
                      ))}
                      {/* TOTAL row for this site */}
                        <tr className="fw-bold bg-light">
                            <td colSpan={2}>Total</td>
                            <td>
                            {site.corrections.reduce((sum, c) => sum + c.total, 0)}
                            </td>
                            <td>
                            {site.corrections.reduce((sum, c) => sum + c.open, 0)}
                            </td>
                            <td>
                            {site.corrections.reduce((sum, c) => sum + c.close, 0)}
                            </td>
                            <td>
                            {(() => {
                                const totalClose = site.corrections.reduce((sum, c) => sum + c.close, 0);
