...
const fetchCompanies = async () => {
  try {
    const res = await api.get("/company/list");  // ✅ replaced with api
...
const handleEdit = async (companyId) => {
  try {
    const res = await api.get(`/company/get/${companyId}`); // use api instance
    if (res.data.status === "success" && res.data.data) {
      const companyData = res.data.data;

      // 🔑 Normalize so "id" is always available
      const normalizedData = {
        ...companyData,
        id: companyData.company_id || companyData.id, // fallback in case API changes
      };

      setEditingCompany(normalizedData);
      setShowForm(true);
    } else {
  ...
  try {
    ...
    const url = isEdit
      ? `/company/update/${editingCompany.company_id}` // ✅ relative URL
      : "/company/register";
